...
label3 <- "Secondary school drop-out rate"


#I create my plot
ggplot(data= third_vis_pivot) + geom_area(aes(x=time,y=rate,fill=index)) + geom_area(aes(x=time,y=rate,fill=index)) + geom_line(aes(x=time,y=cumulative_drop_out_rate_lower_education),colour="#AE0E24") + theme_minimal() +
...
ggplot(datavis_recent_dbl, aes(x= mean_per_subject,y=score,fill=is_morocco)) + geom_col(position = "dodge") + ylim(0,600) + theme_minimal() +
  labs(title = "Morocco performs equally bad in all subjects",subtitle ="Mean PISA score per subject", y = "PISA scores", caption = "Source : worldbank.org/education", fill= "Region") + 
  theme_minimal()  +  
  scale_fill_manual(values = c("#C0392B","#D5F5E3")) +
  theme(plot.title = element_text(face = "bold", size = 19, family = "Oswald"),
      legend.title = element_text(family = "Oswald"),
